...
test("Post a refund twice without idempotency key", async ({ page, request, evidence }) => {
    ...
    const secondRefund = await secondRefundResponse.json();

    expect(secondRefund.refundId).toBe(firstRefund.refundId);
