package com.example.dbframework.config;
...
public record DatabaseConfig(String jdbcUrl, String username, String password) {
    ...
    static DatabaseConfig fromDatabaseUrl(String databaseUrl) {
        ...
        if (!databaseUrl.startsWith(prefix)) {
            throw new IllegalArgumentException("DATABASE_URL must start with mysql:// . current: " + databaseUrl);
        }
    ...
    private static String required(String name) {
        String value = TestEnvironment.required(name);
