...
async fillCheckout(){
    ...
    await this.address.fill('UST Campus, Trivandrum'
    );
    await expect(this.address).toHaveValue('UST Campus, Trivandrum');
...
await Promise.all([
    ...
    this.page.waitForResponse(r=>r.url().includes('/orders') && r.status()===201),
