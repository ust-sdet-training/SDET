...
export class ProductPage {
    ...
    async addToCart(){
        ...
        await Promise.all([this.page.waitForResponse(r=>r.url().includes('/cart/items')&&r.request().method()==='POST'&&r.status()===201),
            this.page.waitForResponse(r=>r.url().includes('/api/cart')&&r.request().method()==='GET'&&r.status()===200),
