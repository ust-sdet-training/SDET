...
public class SecureOrderDatabaseTest extends BaseDatabaseTest {

    @Test
    @DisplayName("Allocate and ship secure order and validate database")
    void secureOrderFlowShouldUpdateDatabase() throws Exception {
        int orderId = given().spec(RequestSpecs.authenticatedSpec(Auth.oauthToken()))
                .body("""
                                {
                                  "items":[101]
                                }
        ...
        given().spec(RequestSpecs.authenticatedSpec(Auth.oauthToken()))
                .when()
