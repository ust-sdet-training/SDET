...
//    }
    ...
    @Test
    @DisplayName("M2: Exercise 2 auth with basic along with spec ")
    void authenticateAWithSpec_Api()
    {
        var response = given()
                .spec(commonJsonRequest)
                . header("x-api-key",TestEnvironment.required("RETAIL_API_KEY"))
//                .header("x-api-key", X_API_KEY)
                ...
                .body("order.orderNumber",equalTo("ORD-5001"))
                .body("order.status",equalTo("Confirmed"))
                .body("order.payment",equalTo("Paid"));
