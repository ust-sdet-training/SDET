...
import com.CI.consumer.contract.OmsClient.Order;
...
class PosOmsConsumerPactTest {
    ...
    @Pact(provider = "oms-provider", consumer = "pos-consumer")
    V4Pact getOrderUnauthorized(PactDslWithProvider builder) {
        return builder
                .given("Order 123 exists but user is unauthorized")
                .uponReceiving("a request for order 123 without valid auth")
                .path("/order/123")
                .method("GET")
                .willRespondWith()
                .status(401)
                .body(new PactDslJsonBody()
                        ...
                        .stringType("message", "Authentication required"))
                .toPact(V4Pact.class);
    }
    @Test
    @PactTestFor(pactMethod = "getOrderUnauthorized")
    void testGetOrderUnauthorized(MockServer mockServer){

        var response = new OmsClient(mockServer.getUrl()).getOrderUnauthorized();

        assertEquals(401, response.statusCode());
        assertEquals(response.path("error"), "Unauthorized");
        assertEquals(response.path("message"), "Authentication required");
    }
