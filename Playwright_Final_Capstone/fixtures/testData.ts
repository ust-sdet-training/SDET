...
const buildBookingData = (): BookingData => {
  ...
  const seatOptions = ['12A', '12B', '12C', '13A', '13B', '13C'] as const;
  const seatPositionOptions = ['left', 'middle', 'right'] as const;

  return {
    ...bookingDataJson,
    employeeId: randomSuffix,
    daysToTravel: 3 + Math.floor(Math.random() * 20),
    seat: seatOptions[Math.floor(Math.random() * seatOptions.length)],
    seatPosition: seatPositionOptions[Math.floor(Math.random() * seatPositionOptions.length)],
  };
};
