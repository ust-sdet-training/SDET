package com.routepulse.api.db.queries;

import com.routepulse.api.db.config.DatabaseConnection;
...
public class BookingQueries {
    ...
    /**
     * Fetches booking by holdId (booking ID)
     */
    public static Map<String, Object> getBookingById(String bookingId) {
        if (connection == null) {
            System.out.println("⚠️  Database not available - skipping DB verification");
            return null;
        }
    ...
    /**
     * Fetches all bookings for a specific employee
     */
    ...
    /**
     * Verifies booking state
     */
    public static String getBookingState(String bookingId) {
        if (connection == null) {
            return null;
        }

        try {
            String query = "SELECT state FROM bookings WHERE id = ?";
            PreparedStatement stmt = connection.prepareStatement(query);
            stmt.setString(1, bookingId);
            ResultSet rs = stmt.executeQuery();
            ...
            System.err.println("Error fetching booking state: " + e.getMessage());
            return null;
        }
    }
