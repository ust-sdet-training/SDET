...
class PosOmsConsumerPactTest {
    ...
    @Pact(provider = "oms-provider", consumer = "pos-consumer")
    V4Pact getForbiddenOrder(PactDslWithProvider builder) {

        return builder
                .given("Order access forbidden")
                .uponReceiving("a request for forbidden order")
                .path("/order/999")
                .method("GET")
                .willRespondWith()
                .status(403)
                ...
                .body(
                        new PactDslJsonBody()
                                .stringType("message", "Forbidden")
                )
                .toPact(V4Pact.class);
    }
    @Test
    @PactTestFor(pactMethod = "getForbiddenOrder")
    void testForbiddenOrder(MockServer mockServer) {

        Response response = new OmsClient(mockServer.getUrl())
                .getForbiddenOrder();
    ...
    @Pact(provider = "oms-provider", consumer = "pos-consumer")
    V4Pact getMissingOrder(PactDslWithProvider builder) {
    ...
    @Test
    @PactTestFor(pactMethod = "getMissingOrder")
    void testMissingOrder(MockServer mockServer) {
